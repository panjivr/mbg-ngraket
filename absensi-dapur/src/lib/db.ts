...
async function doEnsureSchema(): Promise<void> {
  ...
  try {
    ...
    // Paket langganan + masa aktif (monetisasi). Dapur lama default 'pro' (penuh).
    await client.query(`ALTER TABLE sppg ADD COLUMN IF NOT EXISTS paket TEXT NOT NULL DEFAULT 'pro'`);
    await client.query(`ALTER TABLE sppg ADD COLUMN IF NOT EXISTS paket_until DATE`);
    ...
    // Laporan & Checklist Ahli Gizi tersimpan (arsip per tanggal). Struktur sama
    // dengan berita_acara namun tabel terpisah agar arsip gizi & akuntan tidak
    // bercampur — menyimpan konten HTML dokumen yang sudah diisi supaya bisa
    // dibuka/cetak ulang & terdokumentasi (laporan harian/mingguan, monitoring
    // suhu, checklist kebersihan, food waste, rekap PO, HACCP).
    await client.query(`
      CREATE TABLE IF NOT EXISTS laporan_gizi (
        id          SERIAL PRIMARY KEY,
        sppg_id     INTEGER REFERENCES sppg(id) ON DELETE CASCADE,
        slug        TEXT NOT NULL DEFAULT '',
        judul       TEXT NOT NULL DEFAULT '',
    ...
    `);
    await client.query(
      `CREATE INDEX IF NOT EXISTS idx_gizi_sppg_tanggal ON laporan_gizi (sppg_id, tanggal DESC, id DESC)`,
    );
    ...
    // Pengaduan/aspirasi pegawai → manajemen (keluh kesah, penilaian, masalah).
    // user_id NULL = dikirim anonim. status: baru | dibaca | selesai.
    await client.query(`
      CREATE TABLE IF NOT EXISTS pengaduan (
        ...
        balasan      TEXT,
        dibalas_by   INTEGER REFERENCES users(id) ON DELETE SET NULL,
    ...
    if (Number(barangCount.rows[0].c) === 0) {
      ...
      const seed: [string, string, string, number][] = [
        ...
        // Bahan Kering
        ["Beras", "bahan_kering", "kg", 25],
        ["Tepung Terigu", "bahan_kering", "kg", 10],
        ["Tepung Maizena", "bahan_kering", "kg", 3],
        ["Minyak Goreng", "bahan_kering", "liter", 10],
        ["Gula Pasir", "bahan_kering", "kg", 5],
        // Bumbu & Rempah
        ["Garam", "bumbu_rempah", "kg", 3],
        ["Kunyit Bubuk", "bumbu_rempah", "pack", 2],
        ["Bawang Merah", "bumbu_rempah", "kg", 5],
        ["Bawang Putih", "bumbu_rempah", "kg", 5],
        ["Kecap Manis", "bumbu_rempah", "botol", 5],
        ["Saus Tiram", "bumbu_rempah", "botol", 3],
        // Produk Olahan Dasar
        ["Pasta Tomat", "olahan_dasar", "botol", 2],
        ["Agar-agar", "olahan_dasar", "pack", 3],
        ["Perisa Makanan", "olahan_dasar", "botol", 2],
        ["Pewarna Makanan", "olahan_dasar", "botol", 2],
        // Bahan Fermentasi/Pelengkap
        ["Baking Powder", "fermentasi", "pack", 2],
        ["Terasi", "fermentasi", "pack", 2],
        ["SP / Pengembang", "fermentasi", "pack", 2],
        // Protein Hewani
        ["Telur", "protein_hewani", "kg", 10],
        // Susu & Olahannya
        ["Susu Kental Manis (SKM)", "susu_olahan", "kaleng", 5],
        ["Keju", "susu_olahan", "pack", 3],
        ["Margarin", "susu_olahan", "kg", 3],
