...
/** Area sub-admin yang bisa di-scope terpisah dari admin penuh. */
export type AksesArea = "distribusi" | "laporan" | "keuangan" | "gizi";

/**
 * Izinkan admin penuh ATAU sub-admin dengan akses area tertentu. Terima satu
 * area atau daftar area (lolos bila memiliki SALAH SATU). Flag selalu dibaca
 * ulang dari DB (bukan token) agar pemberian/pencabutan akses langsung berlaku
 * tanpa login ulang — token berumur 7 hari jadi flag di token bisa basi.
 */
export async function requireAkses(area: AksesArea | AksesArea[]): Promise<SessionData> {
  ...
  const flag: Record<AksesArea, boolean> = {
  ...
  if (!areas.some((a) => flag[a])) {
    throw new HttpError(403, "Tidak punya akses ke fitur ini.");
  }
  return s;
}
