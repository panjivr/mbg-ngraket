...
/**
 * Arsip Laporan & Checklist Ahli Gizi. Pilih tanggal → tampil semua dokumen
 * gizi yang tersimpan pada tanggal itu. Tiap item bisa dibuka/cetak ulang atau
 * dihapus. Sumber data: /api/admin/ahli-gizi/dok (tabel laporan_gizi).
 */
...
export default function ArsipGiziPage() {
  const [tanggal, setTanggal] = useState("");
  const [items, setItems] = useState<GiziItem[]>([]);
  ...
  const muat = useCallback(async (tgl: string) => {
    ...
    try {
      const res = await fetch(
        `/api/admin/ahli-gizi/dok?tanggal=${encodeURIComponent(tgl)}`,
      );
  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold">🗓️ Arsip Laporan Ahli Gizi</h1>
          <p className="mt-1 text-sm text-slate-400">
            Pilih tanggal untuk melihat semua dokumen gizi yang tersimpan pada
            hari itu.
          </p>
        </div>
      ...
      {!loading && !error && items.length === 0 && (
        <p className="text-sm text-slate-500">
          Tidak ada dokumen tersimpan pada tanggal ini.
        </p>
      )}
      ...
      <div className="grid gap-3">
        {items.map((it) => (
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <a
                href={`/cetak/ahli-gizi/tersimpan/${it.id}`}
