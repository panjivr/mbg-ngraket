...
export const POST = route(async (req: NextRequest) => {
  ...
  if (id !== null) {
    ...
    await catatAudit(admin, "ubah", "Berita Acara", `${judul} (${tanggal})`);
    return ok({ id: upd[0].id });
  }

  const ins = await query<{ id: number }>(
    `INSERT INTO berita_acara (sppg_id, slug, judul, nomor, tanggal, konten_html, dibuat_oleh)
     VALUES ($1, $2, $3, $4, $5, $6, $7)
     RETURNING id`,
    [admin.sppg_id, slug, judul, nomor, tanggal, konten, admin.nama ?? ""],
  );
  await catatAudit(admin, "buat", "Berita Acara", `${judul} (${tanggal})`);
